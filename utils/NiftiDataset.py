...
def create_list_CT(data_path):
    ...
    list_all = [{'data': os.path.join(data_path + 'low_dose/', path), 'CT': os.path.join(data_path + 'CT/', path), 'label': os.path.join(data_path + 'std_dose/', path)} for path in data_list]
...
class NifitDataSet_CT(torch.utils.data.Dataset):
    ...
    def __getitem__(self, item):
        ...
        CT = self.read_image(CT_path)
        CT = Normalization(CT)  # set intensity 0-255
        ...
        castImageFilter.SetOutputPixelType(self.bit)
        CT = castImageFilter.Execute(CT)

        castImageFilter.SetOutputPixelType(self.bit)
        label = castImageFilter.Execute(label)

        sample = {'image': image,'CT': CT, 'label': label,}
        ...
        image_np = abs(sitk.GetArrayFromImage(sample['image']))
        CT_np = abs(sitk.GetArrayFromImage(sample['CT']))
        label_np = abs(sitk.GetArrayFromImage(sample['label']))
        ...
        CT_np = (CT_np - 127.5) / 127.5
        CT_np = CT_np[np.newaxis, :, :, :]

        label_np = (label_np - 127.5) / 127.5
        label_np = label_np[np.newaxis, :, :, :]

        return torch.from_numpy(image_np), torch.from_numpy(CT_np), torch.from_numpy(label_np), tempfilename
