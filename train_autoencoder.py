...
import train.NiftiDataset as NiftiDataset
from train.NiftiDataset import *
...
def run(cfg: DictConfig):
    ...
    trainTransforms = [
            ...
            NiftiDataset.RandomCrop((cfg.patch_size[0], cfg.patch_size[1], cfg.patch_size[2])),
            ]

    train_set = NifitDataSet(train_list, transforms=trainTransforms, train=True)    # define the dataset and loader
    train_dataloader = DataLoader(train_set, batch_size=cfg.model.batch_size, shuffle=True, num_workers=cfg.model.num_workers)  # Here are then fed to the network with a defined batch size
    valTransforms = [
    # NiftiDataset.Resample(opt.new_resolution, opt.resample),
    ...
    val_dataloader = DataLoader(val_set, batch_size= 1, shuffle=False, num_workers=cfg.model.num_workers)
